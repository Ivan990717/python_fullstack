...
4. @staticmethod 和 @classmethod的区别？
   ...
   >@staticmethod：定义时：可以有任意个参数；    执行时：类和对象均可以触发执行
   >
   >@classmethod：定义时：至少有一个cls参数；    执行时：类和对象均可以触发执行，自动把当前类当做参数传递给cls。

5. 简述 `__new__`和`__init__`的区别？

   >`__new__`：**构造**方法，用于创建对象
   >
   >`__init__`:  **初始化**方法，用于在对象中初始化值

6. 在Python中如何定义私有成员？

7. 请基于`__new__` 实现一个单例类（加锁）。

   ![image-20220412153453749](/Users/ivanlee/Library/Application Support/typora-user-images/image-20220412153453749.png)
...
10. 简述 迭代器、可迭代对象 的区别？
    ...
    >```
    >
    >可迭代对象：类中定义了`__iter__`的方法并返回一个迭代器对象

11. 什么是反射？反射的作用？

    >反射就是通过字符串的形式去操作对象中的成员，有四个函数
...
15. 简述你理解的IP和子网掩码。
    ...
    >子网掩码用于划分IP段，IP分为：网络地址 + 主机地址。掩盖住的部分就是网络号，剩下的就是主机号

16. 端口的作用？

    >在网络编程中，IP代指的就是计算机，而端口就指的是计算机中的某个程序，端口可以区分开不同的程序

17. 什么是粘包？如何解决粘包？

    >网络上互相发送数据和接收数据时，分别是吧数据先发到缓冲区以及从过年缓冲区接收数据
    >
    >```python
    >- 对于发送者，执行 `sendall/send` 发送消息时，是将数据先发送至自己网卡的 写缓冲区 ，再由缓冲区将数据发送给到对方网卡的读缓冲区。
    >- 对于接受者，执行 `recv` 接收消息时，是从自己网卡的读缓冲区获取数据。
...
19. 简述进程、线程、协程的区别。
    ...
    >进程：是计算机中可以**被cpu调度**的最小单元。
    >
    >线程：是计算机**资源分配**的最小单元（进程为线程提供资源）
    >
    >​			由于Cpython中的GIL机制，线程更适合于IO密集型操作，进程适合计算密集型操作
    >
...
21. 进程之间如何实现数据的共享？
    ...
    >```python
    >ultiprocessing.Value 或 multiprocessing.Array
...
23. 基于面向对象的知识构造一个链表。
    ...
    ![image-20210428164850308](/Users/ivanlee/Desktop/前3模块/day24 阶段总结和考试/考试题和参考答案/assets/image-20210428164850308.png)
...
25. 请自己基于socket模块和threading模块实现  门票预订 平台。（无需考虑粘包）
    ...
    - socket服务端，可以支持并发多人同时查询和购买。（为每个客户度创建一个线程）。
      ...
      - 服务端数据存储结构如下：
        ...
        ![image-20210428133641127](/Users/ivanlee/Desktop/前3模块/day24 阶段总结和考试/考试题和参考答案/assets/image-20210428133641127.png)

        ![image-20210428133822091](/Users/ivanlee/Desktop/前3模块/day24 阶段总结和考试/考试题和参考答案/assets/image-20210428133822091.png)
