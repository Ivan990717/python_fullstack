import logging
logging.basicConfig(level=logging.DEBUG,
                    format='%(asctime)s - %(filename)s[line:%(lineno)d] - %(levelname)s: %(message)s')  # logging.basicConfig函数对日志的输出格式及方式做相关配置
# 由于日志基本配置中级别设置为DEBUG，所以一下打印信息将会全部显示在控制台上
import re,socket,time,os,json
from _client._config import common
from _client.utils import req

class Handler():
    def __init__(self):
        self.host = common.ip_address
        self.port = common.port
        self.conn = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.username = None

    def run(self):
        self.conn.connect((self.host,self.port))
        welcome = """
               登录：login 用户名 密码
               注册：register 用户名 密码
               查看：ls 目录
               上传：upload 本地目录 远程目录
               下载：download 本地目录 远程目录 
               """
        print(welcome)

        method_map = {
            "login": self.login,
            "register": self.register,
            "ls": self.ls,
            "upload": self.upload,
            "download": self.download,
        }
        while True:
            tip = '{}>>>'.format(self.username or '未登录')
            text = input(tip).strip()
            if not text:
                logging.warning("输入不能为空，请重新输入。")
                continue

            if text.upper() == "Q":
                logging.info("退出")
                req.send_data(self.conn, "q")
                break

            cmd, *args = re.split('\s+',text)
            cmd_res = method_map.get(cmd)
            if not cmd_res:
                logging.error("命令不存在，请重新输入。")
                continue
            cmd_res(*args)

        self.conn.close()

    def login(self,*args):
        ''' correct input: login username password'''
        if len(args) !=2:
            logging.warning("格式错误，请重新输入。提示：login 用户名 密码")
            return
        username, password = args
        req.send_data(self.conn, "login {} {}".format(username, password))
        reply = req.recv_data(self.conn).decode('utf-8')
        reply_dict = json.loads(reply)
        if reply_dict['status']:
            self.username = username
            logging.info("登录成功")
            return
        print(reply_dict['error'])

    def register(self,*args):
        if len(args) != 2:
            logging.error("格式错误，请重新输入。提示：register 用户名 密码")
            return
        username, password = args
        req.send_data(self.conn, "register {} {}".format(username, password))
        reply = req.recv_data(self.conn).decode('utf8')
        reply_dict = json.loads(reply)
        if reply_dict['status']:
            logging.info("注册成功")
            return
        print(reply_dict['error'])
    def ls(self,*args):
        if not self.username:
            logging.warning("未登录")
        if not args:
            cmd = "ls"
        elif len(args) == 1:
            cmd = "ls {}".format(*args)
        else:
            logging.warning(
                logging.error("格式错误，请重新输入。提示：ls 或 ls 目录 "))
            return
        req.send_data(self.conn, cmd)
        reply = req.recv_data(self.conn).decode('utf-8')
        reply_dict = json.loads(reply)
        if reply_dict['status']:
            logging.info(print(reply_dict['data']))
            return
        logging.error(reply_dict['error'])
    def upload(self,*args):
        if not self.username:
            logging.warning(("登录后才允许上传"))
            return
        if len(args) != 2:
            logging.warning(("格式错误，请重新输入。提示：upload 本地目录 远程目录"))
            return
        local_file_path, remote_file_path = args
        if not os.path.exists(local_file_path):
            logging.error('文件{}不存在'.format(local_file_path))
            return

        req.send_data(self.conn, "upload {}".format(remote_file_path))

        reply = req.recv_data(self.conn).decode('utf-8')
        reply_dict = json.loads(reply)
        if not reply_dict['status']:
            logging.error(reply_dict['error'])
            return

        logging.info("开始长传")
        # 开始上传文件
        req.send_file(self.conn, local_file_path)

        logging.info("上传完毕")

    def download(self,*args):
        if not self.username:
            logging.warning("登录后才允许下载")
            return
        if len(args) != 2:
            logging.error("格式错误，请重新输入。提示：download 本地目录 远程目录")
            return

        local_file_path, remote_file_path = args
        seek = 0
        if not os.path.exists(local_file_path):
            # download v1.txt
            req.send_data(self.conn, "download {}".format(remote_file_path))
            mode = 'wb'
        else:
            choice = input("是否续传（Y/N) ")
            if choice.upper() == 'Y':
                # download v1.txt 100
                seek = os.stat(local_file_path).st_size
                req.send_data(self.conn, "download {} {}".format(remote_file_path, seek))
                mode = 'ab'
            else:
                # download v1.txt
                req.send_data(self.conn, "download {}".format(remote_file_path))
                mode = 'wb'

        reply = req.recv_data(self.conn).decode('utf-8')
        reply_dict = json.loads(reply)
        if not reply_dict['status']:
            print(reply_dict['error'])
        else:
            logging.info("开始下载")  # print(reply_dict['data'])
            req.recv_save_file(self.conn, local_file_path, mode, seek=seek)
            logging.info("下载完毕")

client_ = Handler()





